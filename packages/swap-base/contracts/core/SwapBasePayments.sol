...
abstract contract SwapBasePayments is ISwapBasePayments {
    ...
    receive() external payable {
        require(msg.sender == s._WETH9, "Not WETH9");
    }
...
//No need: No ETH Support
    ...
    function _pay(
        ...
        if (token == s._WETH9 && IERC20(s._WETH9).balanceOf(address(this)) >= value) {
            // pay with WETH9
            IWETH9(s._WETH9).deposit{value: value}(); // wrap only what is needed to pay
            IWETH9(s._WETH9).transfer(recipient, value);
